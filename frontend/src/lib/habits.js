...
export function isScheduledForDate(habit, dateStr) {
...
export function isCompletedToday(habitId, logs) {
    const today = todayKey();
    return logs.some((l) => l.habitId === habitId && l.date === today);
}
export function toggleHabit(habitId, logs) {
    const today = todayKey();
...
export function getStreak(habitId, logs, habits) {
    ...
    if (habitLogs[0] !== today && habitLogs[0] !== yesterdayKey)
        return 0;
...
export function getLongestStreak(habitId, logs) {
    const dates = logs
        .filter((l) => l.habitId === habitId)
        .map((l) => l.date)
        .sort();
...
export function getCompletionRate(habitId, logs, habit) {
...
export function getWeeklyCompletionCount(habitId, logs) {
    const today = new Date();
...
export function getLast30DaysMap(habitId, logs) {
    const map = new Map();
...
export function createHabit(name, description, color, frequency = { type: "daily" }, reminderTime = null) {
    return {
...
export function updateHabit(habits, updated) {
